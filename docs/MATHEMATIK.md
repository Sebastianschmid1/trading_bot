...
Signal-Stärke** S ∈ [0, 100], die etablierte technische Indikatoren (RSI, MACD, Trend, Volumen)
über mehrere Zeiträume (5m, 15m, 1h, 1d) verdichtet. Risiko wird über **ATR-basierte**
Stop-Loss-/Take-Profit-Grenzen, einen wählbaren **Hebel** mit zugehörigem **Liquidationskurs** und
eine auf die Margin begrenzte **P&L-Funktion** gesteuert. Ergänzend wird ein **Smart-Money-Score**
aus öffentlich gemeldeter Insider- und Institutionen-Aktivität gebildet und in das Ranking
eingerechnet. Dieses Papier stellt die zugrunde liegende Mathematik vollständig und nachvollziehbar dar.
...
- Pₜ den Schlusskurs (Close) zum Bar-Index t, analog Hₜ (High), Lₜ (Low), Vₜ (Volumen);
- N die Anzahl verfügbarer Bars eines Zeitraums;
- 𝒯 = {5m, 15m, 1h, 1d} die Menge der **Zeiträume** (Timeframes) mit zugehörigen
  Aggregationsgewichten ω(tf).

Die Daten werden je Zeitraum als **ein** Batch-Download geladen (`analyzer._download_all_timeframes`);
pro Ticker entsteht ein OHLCV-DataFrame je Zeitraum tf ∈ 𝒯. Das System ist im Demo-Modus
...
Aus den Kursdifferenzen Δₜ = Pₜ − Pₜ₋₁ werden Gewinne und Verluste getrennt:
...
Konvention: ist der mittlere Verlust null, gilt RSI = 100; bei zu wenigen Bars wird neutral 50
zurückgegeben. Schwellen: RSI < 35 = überverkauft (bullish), RSI > 65 = überkauft.

### 3.2 MACD — Moving Average Convergence/Divergence *(→ `analyzer.calc_macd`)*

Mit dem exponentiellen gleitenden Durchschnitt (EMA) der Spanne s, Glättungsfaktor α = 2/(s+1),
...
Ein **bullishes** Setup liegt vor, wenn MACDₜ > Signalₜ **und** Histₜ > 0.
...
Die täglichen Schlusskurse werden auf Wochenbasis resampelt: Wₖ = letzter Schluss der Woche k.
...
gilt der Trend als **up**, falls Wₖ über beiden Mitteln und das Mittel jetzt ≥ Mittel zuvor liegt;
als **down**, falls Wₖ unter beiden Mitteln und das Mittel jetzt ≤ Mittel zuvor liegt; sonst **flat**.
...
Abstand die Toleranz τ = 0,02 (2 %) nicht überschreitet; jedem Cluster C wird sein
...
(je höher, desto bullisher). Mit der Begrenzungsfunktion

$$
\mathrm{clip}(x) = \min(\max(x,\,0),\;\, 1):
$$

**RSI-Score** (überverkauft ⇒ stark):

$$
s_{\text{rsi}} = \mathrm{clip}\!\left(\frac{65 - \text{RSI}}{35}\right). \qquad (12)
$$
...
**Trend-Score** (Lage zu den 20- und 50-Bar-Durchschnitten des Zeitraums):
...
**Volumen-Score** über das relative Volumen RVOL (aktuelles Volumen geteilt durch den
20-Bar-Durchschnitt), voll bestätigt ab etwa 1,5×:

$$
s_{\text{vol}} = \mathrm{clip}\!\left(\frac{\text{RVOL} - 0{,}8}{0{,}7}\right). \qquad (15)
$$

Der **Timeframe-Score** ist das mit den Komponenten-Gewichten (`STRENGTH_WEIGHTS`)
...
Die Gesamt-Stärke ist das mit den Zeitraum-Gewichten ω(tf) (`SIGNAL_TIMEFRAMES`) gewichtete Mittel
der verfügbaren Timeframe-Scores (fehlende Zeiträume werden übersprungen und die Gewichte über die
vorhandenen normiert):
...
einen Intraday-Horizont (Trade ≤ 1 Tag) am prädiktivsten sind. (ii) *Multi-Timeframe-Confluence*
...
dem 1d-Zeitraum für das Gate, Stärke aus der Multi-TF-Aggregation) — mit der Mindeststärke
55 (`MIN_SIGNAL_STRENGTH`):
...
modusabhängigen Faktoren mₛₗ (Stop-Loss) und mₜₚ (Take-Profit) aus `SL_TP_MODES`:
...
geschlossen. Der Stop-Loss liegt prozentual unter, das Take-Profit über dem Einstieg.
...
Für L ≤ 1 existiert keine Liquidation. **Konsequenz:** Je höher der Hebel, desto näher liegt der
Liquidationskurs am Einstieg — bei L = 10 genügt bereits ein Rückgang von 10 %, bei L = 2 erst 50 %.
Höherer Hebel vergrößert also Gewinn **und** Verlust und führt zu deutlich **früherer** Liquidation.

---

## 8. P&L — Gewinn und Verlust *(→ `evaluator.realized_pnl`)*

Die reine Kursrendite (ungehebelt) beim Ausstieg zum Kurs Pₑₓᵢₜ:
...
tatsächlichen Liquidations-Exit der Ausstiegskurs auf den Liquidationskurs gesetzt (Abschnitt 9).

---

## 9. Schließlogik *(→ `bot.evaluate_active_trade`, `evaluator.evaluate_trades`)*

Ein aktiver Trade wird in **fester Prioritätsreihenfolge** geprüft (schlimmster Fall zuerst), mit der
Schließschwelle 35 (`SIGNAL_CLOSE_THRESHOLD`). Es bezeichne P den aktuellen Kurs und S die aktuelle Stärke:

1. P ≤ Liquidationskurs &nbsp;⇒&nbsp; **Liquidation** 💥
2. P ≤ Stop-Loss &nbsp;⇒&nbsp; **Stop-Loss** 🛑
3. P ≥ Take-Profit &nbsp;⇒&nbsp; **Take-Profit** 🎯
4. S < 35 &nbsp;⇒&nbsp; **Signal-Verfall** 📉
...
Hebel berechnet; beim Liquidations-Exit ist der Ausstiegskurs gleich dem Liquidationskurs.
...
SM ∈ [0, 100] als Summe dreier Komponenten gebildet.

**Insider-Komponente** [0, 50], neutral = 25 (SEC Form 4, ~2 Tage Verzug). Aus gekauften/verkauften
Stückzahlen Bₛ, Sₛ und der Zahl der Kauf-/Verkaufs-Transaktionen Bₙ, Sₙ wird ein Netto-Signal
in [−1, 1] gemittelt:

$$
\text{sig}_{\text{ins}} = \mathrm{mean}\!\left(\frac{B_s - S_s}{B_s + S_s},\;\; \frac{B_n - S_n}{B_n + S_n}\right),
\qquad
...
Bestandsänderung (auf [−1, 1] geklippt) und dem Verhältnis aufstockender (a) zu reduzierenden (r)
Haltern bei m Haltern insgesamt:

$$
\text{sig}_{\text{inst}} = \mathrm{mean}\!\left(\mathrm{clip}_{[-1,1]}(\overline{\Delta}),\;\; \frac{a - r}{m}\right),
\qquad
C_{\text{inst}} = 20\,\bigl(1 + \text{sig}_{\text{inst}}\bigr). \qquad (25)
$$

**Niveau-Komponente** [0, 10] aus dem institutionellen Besitzanteil π ∈ [0, 1]. Der Gesamtscore und
die Sterne-Darstellung:

$$
C_{\text{lvl}} = 10\cdot\mathrm{clip}_{[0,1]}(\pi), \qquad
\text{SM} = \mathrm{round}\Bigl(\mathrm{clip}_{[0,100]}\bigl(C_{\text{ins}} + C_{\text{inst}} + C_{\text{lvl}}\bigr)\Bigr). \qquad (26)
$$

Die Sterne ergeben sich als auf 1…5 begrenzter gerundeter Wert von SM/20.
...
| `SIGNAL_TIMEFRAMES` — Gewichte ω(tf) | 0,40 / 0,30 / 0,20 / 0,10 | 5m / 15m / 1h / 1d |
| `STRENGTH_WEIGHTS` — Gewichte w(c) | 0,20 / 0,25 / 0,25 / 0,30 | rsi / macd / trend / **volume** |
| `MIN_SIGNAL_STRENGTH` | 55 | Mindeststärke für ein Signal |
| `SIGNAL_CLOSE_THRESHOLD` | 35 | Auto-Close bei Signal-Verfall |
| `SL_TP_MODES` — Faktoren (SL; TP) | aus / (1,0; 1,5) / (1,5; 2,5) / (2,5; 4,0) | aus / passiv / normal / aggressiv |
| `LEVERAGE_CHOICES` — Hebel L | 1; 1,5; 2; 3; 5; 10 | wählbarer Hebel |
