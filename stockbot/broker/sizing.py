...
def plan_share_order(price: float, budget: float) -> dict:
    """Plant einen Aktienkauf fürs Budget.

    - Budget reicht für ≥ 1 ganze Aktie → ganze Aktien (keine Bruchteile).
    - Aktie teurer als das Budget → genau ein Bruchteil über `notional = budget`
      (einzig erlaubter Bruchteil-Fall).
    ...
    if whole >= 1:
        return {"kind": "shares", "qty": whole, "notional": None, "fractional": False}
    # Eine einzelne Aktie ist teurer als das gesamte Budget → Bruchteil zulassen.
    return {"kind": "shares", "qty": None, "notional": round(budget, 2), "fractional": True}


def plan_order(price: float, trade_size: float, leverage: float, *,
               option_selector=None, extended: bool = False) -> dict:
    ...
    return plan_share_order(price, budget)
