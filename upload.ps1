...
try {
    ...
    $tmpl = 'cd {APP} && ASK=$(mktemp) && printf ''#!/bin/sh\nprintf %%s "$DEPLOY_PASS"\n'' > "$ASK" && chmod +x "$ASK" && DEPLOY_PASS="$(printf %s ''{PASS}'' | base64 -d)" SSH_ASKPASS="$ASK" SSH_ASKPASS_REQUIRE=force DISPLAY=:0 git -c safe.directory={APP} pull < /dev/null; rc=$?; rm -f "$ASK"; [ $rc -eq 0 ] && { chown -R stockbot:stockbot {APP}; runuser -u stockbot -- {APP}/venv/bin/pip install -q -r {APP}/requirements.lock || echo "WARN: pip install fehlgeschlagen"; bash deploy/sync_caddy.sh || echo "WARN: caddy-sync fehlgeschlagen"; systemctl restart stockbot; systemctl status stockbot --no-pager -l | head -n 15; echo "==== stockbot Logs (30s) ===="; timeout 30 journalctl -u stockbot -n 20 -f || true; }; exit $rc'
